...
export function DistributorManagement() {
  ...
  const fetchDistributors = useCallback(async () => {
    ...
    if (error) setError("Gagal memuat data distributor. Coba refresh halaman.");
  ...
  useEffect(() => {
    ...
    const channel = supabaseAdmin
      ...
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "distributors" },
        () => {
          fetchDistributors();
        },
      )
  ...
  const handleApprove = async (userId: string) => {
    ...
    if (error) alert("Gagal menyetujui distributor: " + (error as any).message);
    else
  ...
  const handleReject = async (userId: string) => {
    ...
    if (error) alert("Gagal menolak distributor: " + (error as any).message);
    else
      setDistributors((prev) =>
        prev.map((d) =>
          d.users.id === userId
  ...
  const handleDelete = async (userId: string) => {
    ...
    if (error) alert("Gagal menghapus distributor: " + (error as any).message);
    else setDistributors((prev) => prev.filter((d) => d.users.id !== userId));
    setActionLoading(null);
  };
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-start gap-4 flex-1 min-w-0">
                        <div
                          className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-lg flex-shrink-0 ${dist.users.is_approved ? "bg-blue-600 text-white" : "bg-orange-400 text-white"}`}
                        >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            ...
                            <span
                              className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${dist.users.is_approved ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700"}`}
                            >
                              {dist.users.is_approved ? (
                                <>
